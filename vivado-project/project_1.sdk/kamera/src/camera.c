...
void InitYUV(void){
		int reg_com7 = ReadReg(REG_COM7);
	    WriteReg(REG_COM7,reg_com7|0x0);
	    int i = 1, status = 0;
	    while (1)
	    	{
	    		if(yuv442[i].reg_num == 0xff && yuv442[i].value == 0xff)
	    			break;
	    		status = WriteReg(yuv442[i].reg_num, yuv442[i].value);
	    		if (status != XST_SUCCESS)
	    			return;
	    		i++;
	    	}
...
void InitDefaultRegValue(void){
		    ...
		    while (1)
		    	{
		    		if(def_regs[i].reg_num == 0xff && def_regs[i].value == 0xff)
		    			break;
		    		status = WriteReg(def_regs[i].reg_num, def_regs[i].value);
		    		if (status != XST_SUCCESS)
		    			return;
		    		i++;
		    	}
